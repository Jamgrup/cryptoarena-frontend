...
export function UserNFTCollection({ className = '' }: UserNFTCollectionProps) {
  ...
  const fetchUserCards = async () => {
    ...
    try {
      ...
      // First, get collection info to know total cards
      const backendUrl = process.env.NEXT_PUBLIC_API_URL || 'https://cryptoarena-backend.onrender.com';
      const collectionResponse = await fetch(`${backendUrl}/api/v1/nft/collection/info`);
      ...
      const totalCards = parseInt(collectionData.data.totalMinted || '0');
      const userCards: NFTCard[] = [];

      // Check each card to see if user owns it
      for (let i = 0; i < totalCards; i++) {
        try {
          const cardResponse = await fetch(`${backendUrl}/api/v1/nft/card/${i}/metadata`);
          ...
          if (cardResponse.ok) {
            ...
            if (cardData.success && (ownerAddress === userAddress || cardData.data.owner === address)) {
              userCards.push({
                ...
                owner: cardData.data.owner,
                attributes: cardData.data.attributes,
                metadata: {
                  name: cardData.data.name,
                  description: cardData.data.description,
                  lore: cardData.data.lore,
                  rarity: cardData.data.rarity,
                  powerRating: cardData.data.powerRating
                },
                explorer: cardData.data.explorer,
                tonviewer: cardData.data.tonviewer
              });
            }
          }
        } catch (cardError) {
          console.warn(`Error checking card ${i}:`, cardError);
          continue;
        }
      }
    ...
    <div className={`p-6 bg-white border border-gray-200 rounded-lg shadow-sm ${className}`}>
      ...
      {loading && (
        <div className="flex items-center justify-center h-32">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-600">Loading your NFT cards...</p>
          </div>
        </div>
      )}
      ...
      {!loading && !error && cards.length === 0 && (
        <div className="text-center py-8">
          ...
          <h4 className="text-lg font-medium text-gray-900 mb-2">No NFT Cards Found</h4>
          <p className="text-gray-600 mb-4">You don't own any NFT cards yet.</p>
          <p className="text-sm text-gray-500">Mint your first card above to get started!</p>
        </div>
      )}
          ...
          <div className="bg-white rounded-lg max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-gray-900">NFT Card #{selectedCard.index}</h3>
                <button
